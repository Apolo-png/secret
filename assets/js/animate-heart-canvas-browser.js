...
class AnimateHeartCanvas {
  ...
  isMouseIsCloseToBox(box) {
    let distance = Math.sqrt(
      Math.pow(Math.abs(box.position.x / 2 - this.mouseX), 2) + Math.pow(Math.abs(box.position.y / 2 - this.mouseY), 2)
    );
    return distance < 100;
  }
  ...
  draw() {
    ...
    this.heartBuffer.forEach((heart) => {
      ...
      // speed = 1/2 gt
      let movement =
        (((1 / 2) * heart.gravity * (this.configHeart.timeLine - heart.timeInit)) / 300) *
